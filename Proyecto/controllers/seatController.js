...
class SeatController {
    static async createSeat(req, res, next) {
        try {
            ...
            const sql = 'INSERT INTO seats (idPlane, number, classType, state, price) VALUES (?, ?, ?, ?, ?)';
            const values = [idPlane, number, classType, state, price];
            await db.query(sql, values);
            res.status(201).json({ message: 'Seat created successfully' });
        } catch (error) {
            next(new Error('Error creating seat'));
        }
    }
    static async createSeat(req, res, next) {
        try {
